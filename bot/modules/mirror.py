...
class MirrorListener:
    ...
    def onUploadComplete(self, link: str, size, files, folders, typ, name: str):
        ...
        reply_to = self.message.reply_to_message
        slmsg = f"𝗟𝗢𝗚𝗚𝗘𝗥\n\n𝑼𝒔𝒆𝒓: {uname} \n𝑼𝒔𝒆𝒓 𝑰𝑫: <code>{self.user_id}</code>\n𝗪𝗮𝗿𝗻: <code>/warn {self.user_id}</code>\n\n𝑳𝒊𝒏𝒌 𝑺𝒆𝒏𝒅𝒆𝒅:"
        if LINK_LOGS:
            try:
        ...
        S_link =  f"https://t.me/c/{link_id}/{msg_id}"
            '''
        msg = f'<b>𝗡𝗮𝗺𝗲: </b><code>{name.replace("<", "")}</code>\n\n<b>𝗦𝗶𝘇𝗲: </b>{size}'
        ...
        if BOT_PM and self.message.chat.type != 'private':
            ...
            pmwarn = ''
            pmwarn_mirror = ''
        logwarn = f"\n<b>𝗙𝗶𝗹𝗲 𝗦𝗲𝗻𝗱𝗲𝗱 𝗧𝗼 𝗟𝗼𝗴 𝗖𝗵𝗮𝗻𝗻𝗲𝗹 😎</b>"
        if self.isLeech:
            count = len(files)
            ...
            msg += f'\n\n<b>𝗥𝗲𝗾 𝗕𝘆: </b>{self.tag}\n'
            if BOT_PM:
                message = sendMessage(msg + pmwarn, self.bot, self.update)
            ...
            if MIRROR_LOGS:
                try:
                    for i in MIRROR_LOGS:
                        bot.sendMessage(chat_id=i, text=msg + uploader + msg_g,
            ...
            if self.isQbit and QB_SEED and not self.extract:
                if self.isZip:
                    try:
                        ...
                        pass
                msg = sendMessage(msg + uploader + pmwarn_mirror, self.bot, self.update)
    ...
    if FSUB:
        try:
            ...
            if user.status not in ('member', 'creator', 'administrator'):
                ...
                message = sendMarkup(
                    str(f"️<b>Dear {uname}, You Need Join our Updates Channel to use our Bot .</b>"),
